...
## Using `reason` from Bash

The `reason` CLI brings the same bounded-judgment pattern to shell scripts and Claude Code hooks. It reads an observation, applies structured prompts, and returns typed JSON — keeping noisy output local instead of promoting it to the outer context.

### Syntax

```bash
# Pipe stdin as observation
...
- **Structure is a JSON example**, not a Schema. Types are inferred from the example values: `""` → string, `0` → int, `false` → bool, `[""]` → string array.
