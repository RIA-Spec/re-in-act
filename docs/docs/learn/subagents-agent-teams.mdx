...
- The **RAS** absorbs environment disturbances in the action phase under that structure.
...
Re in Act is therefore not a multi-agent hierarchy. It keeps one top-level reasoning context and a programmable local action space.
...
The action environment is intentionally Turing-complete because local work often needs loops, branches, retries, and validation to happen inside the same control envelope. If those steps cannot be expressed locally, they leak back into outer-loop reasoning and you lose the benefit of local regulation.
