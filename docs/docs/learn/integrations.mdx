...
One useful reference design here is Anthropic's [Programmatic tool calling](https://platform.claude.com/docs/en/agents-and-tools/tool-use/programmatic-tool-calling). In that model, Claude writes code inside a code-execution container, calls tools from the running program, pauses on `tool_use`, resumes when the host returns `tool_result`, and keeps intermediate processing out of the model's main context window until the final output is ready.

That is close to the Re in Act intuition: loops, filtering, branching, and local tool coordination should happen **inside execution**, not through repeated outer-loop model turns.

Re in Act is broader than that specific product feature. The spec does not require Anthropic-managed code execution or Anthropic's response protocol. `reason()` can be backed by any LLM API that supports structured output and **fresh-context** calls, while `act()` can be wired to MCP tools, custom functions, or runtime-native actions.

Recommended models for `reason()` calls: lightweight, fast models. `reason()` is an atomic local decision step, not a full main reasoning pass — it does not need frontier-scale capability.
