...
For `agent()`-style delegated execution, ACP is a recommended integration layer.

For the Re in Act `agent()` extension contract itself, see [Agent Interface Extension](/extensions/agent-interface).
...
- use **MCP** to discover and call tools inside the RAS
- use **ACP** to connect and orchestrate agent runtimes
