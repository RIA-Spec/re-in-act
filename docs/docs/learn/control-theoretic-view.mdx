...
Put differently: if execution has no local model, the top-level reasoning loop must keep rebuilding one through tokens. That is expensive, slow, and fragile.
...
- then produce a bounded control decision that execution can use immediately.
