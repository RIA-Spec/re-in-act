...
Here is a minimal example of **reason in action** as progressive discovery.

The agent does not decide all tool usage upfront. It first asks `act --manual` for the available tool surface, lets `reason` pick only the tool names relevant to the task, then asks `act --manual [name]` for the detailed definitions of just those tools.
...
It helps to be explicit about the boundary.
