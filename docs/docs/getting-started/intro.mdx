...
Top-level reasoning should define the work and its constraints. The action layer should handle environment disturbances inside that bounded action space instead of escalating every small issue back to the outer loop.
...
- **Ashby's Law (Requisite Variety)**: only variety can absorb variety. ReAct's fixed action layer often cannot express enough differentiated responses to handle environment disturbances well. The RAS expands that local controller variety.
- **Good Regulator Theorem**: a successful regulator needs access to a model of the system it regulates. In Re in Act, the RAS gives the local controller a model of the local system it regulates, so the agent does not need to rebuild that model from scratch on every outer-loop turn.
...
Here is a minimal example of **reason in action** as progressive discovery: the agent first asks `act --manual` for the available tool surface, let `reason` pick only the relevant tool names, then asks `act --manual [name]` for the detailed definitions of just those tools.
...
- **Not a subagent pattern** — Re in Act does not create another reasoning actor. It creates one orchestrated action space for the same agent. This is a layered control architecture rather than a multi-agent hierarchy.
