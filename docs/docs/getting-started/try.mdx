...
When using Bash tool, send noisy local output to `reason` instead of carrying it in the main context window.

Use `reason` for a bounded local judgment:

- one `--prompt` for the goal
- one `--prompt` for rules
- `--prompt -` for stdin observation
- `--structure` for a typed result that Bash can keep piping

Use `reason` for things like classification, extraction, prioritization, or next-step selection.
Keep loops, retries, branching, and tool execution in Bash.
```
