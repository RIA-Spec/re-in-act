...
The recommended success envelope is `{ data: { text, trajectory } }`, so `reason()` can validate both the primary result text and the execution trajectory before producing downstream control signals.

On success, `agent()` SHOULD return `{ data: { text, trajectory } }`. If structured validation is needed, the RAS SHOULD run a follow-up `reason()` call to verify and normalize both fields into schema-bounded data for downstream control flow.
