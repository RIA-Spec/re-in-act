...
In this model, the RAS is the harness: `agent()` performs delegated work and returns text, `reason()` checks and normalizes that text, and runtime control flow enforces deterministic limits such as iteration count, timeout, and escalation.

On success, `agent()` returns text output. If structured validation is needed, the RAS SHOULD run a follow-up `reason()` call to verify and normalize that text into schema-bounded data for downstream control flow.
