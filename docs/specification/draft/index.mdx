...
<Note>
  This is a **working draft** (2026-04-19), open for public review. Author:
  [RIA-Spec](https://github.com/RIA-Spec).
</Note>
...
### 6.2 `agent()` as an Optional Extension
...
For reference, we open-sourced a Re in Act implementation project called [one-agent](https://github.com/RIA-Spec/one-agent).

- Supports Bash RAS and Code RAS modes, with `reason()` implemented across Bash, Python, and TypeScript runtimes
- Uses MCP for tool discovery and execution as one implementation of the `act()` interface
- Implements the optional `agent()` extension and Riff for reusable harness-style RAS programs
