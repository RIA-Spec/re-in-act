...
- `reason()` checks `text` and `trajectory`, extracts structured signals, and decides next control action
...
2. Structured success envelope:
   On success, `agent()` returns `{ data: { text, trajectory } }`. `trajectory` MUST follow ATIF. Structured validation and normalization happen in a separate `reason()` step, and `trajectory` can be used as an additional verification signal.
