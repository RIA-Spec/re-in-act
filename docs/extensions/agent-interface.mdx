...
2. Structured success envelope:
   On success, `agent()` returns `{ data: { text, trajectory } }`. Structured validation and normalization happen in a separate `reason()` step, and `trajectory` can be used as an additional verification signal.
